...
data_for_connection = ("DRIVER={ODBC Driver 17 for SQL Server};Server=JOSECOSTA;Database=bncc;Trusted_Connection=yes;"
) 
...
def list_filters():
    ...
    filter_cla = request.values.get('turma')

    if filter_cla is not None:
        query_l = cursor.execute(f""" 
            SELECT * FROM tabela_alunos WHERE id_turma = {filter_cla} """)
